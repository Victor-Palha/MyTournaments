...
describe("create card use case", () => {
    let sut: CreateCardsUseCase

    beforeEach(()=>{
        sut = new CreateCardsUseCase()
    })

    it("should be able to create a deck list", async () => {
        const mainDeck: CardProps[] = [{
        ...
        const {
            ...
            total_side_deck,
            total_extra_deck
        } = await sut.execute({main_deck: mainDeck, side_deck: sideDeck, extra_deck: extraDeck})

        expect(total_main_deck).toBe(40)
        expect(total_side_deck).toBe(14)
        expect(total_extra_deck).toBe(8)
    })
})